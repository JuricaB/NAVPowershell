...
$SourceFolder = 'SPECIFYFOLDER\Work'
...
foreach ($file in $files)
{
    ...
    $json_convert.value = $json_convert.value | Select-Object * -ExcludeProperty 'Entry_No'
    $json_convert.value = $json_convert.value | Select-Object * -ExcludeProperty 'Correlation_ID'
    ...
    $json_convert.value = $json_convert.value | Select-Object * -ExcludeProperty 'Created_Date_Time'
    $json_convert.value = $json_convert.value | Select-Object * -ExcludeProperty 'Processing_Duration'
    $json_convert.value = $json_convert.value | Select-Object * -ExcludeProperty 'Integration_Run_ID'
     ...
     foreach ($element in $json_convert.value) {
         ...
         if (($counter % 100) -eq 0) { 
            ...
            (Get-Content $pathToConvertedJson).Replace('"Direction"', '"direction"') | Set-Content $pathToConvertedJson
            (Get-Content $pathToConvertedJson).Replace('"Source"', '"source"') | Set-Content $pathToConvertedJson
            (Get-Content $pathToConvertedJson).Replace('"Message_Type"', '"messageType"') | Set-Content $pathToConvertedJson
            (Get-Content $pathToConvertedJson).Replace('"Destination"', '"destination"') | Set-Content $pathToConvertedJson
            (Get-Content $pathToConvertedJson).Replace('"BodyText"', '"bodyText"') | Set-Content $pathToConvertedJson
         }
