function HowTo-ExportNAVApplicationDatabase
{
<# 
.SYNOPSIS
This Windows Powershell script shows how to extract the application-wide tables in a Microsoft Dynamics NAV database to a separate database and migrate any database users that may exist. The new application database is created on the same SQL Server instance as the original database.

.DESCRIPTION
The HowTo-ExportNAVApplicationDatabase cmdlet copies application-wide tables in a Microsoft Dynamics NAV database to a new, dedicated application database and migrates any database users that may exist. The application database is created on the same SQL Server instance as the original database.
...
.PARAMETER DatabaseName
  Specifies the name of the Microsoft Dynamics NAV database that contains the application tables. This database is the original database that application-wide tables will be extracted from.
.PARAMETER DestinationDatabaseName
  Specifies the name of the destination database. If the destination database does not exist it is created on the same SQL Server instance as the original database.
.PARAMETER ServiceAccount
  Logon account that runs the Microsoft Dynamics NAV Server instance.

.EXAMPLE
HowTo-ExportNAVApplicationDatabase -DatabaseServer MyServer -DatabaseInstance NavDemo -DatabaseName 'Demo Database NAV (7-0)' -DestinationDatabaseName 'NAV App' 
...
#>
    ...
    BEGIN
    {
        SafeAddPSSnapin "Microsoft.Dynamics.Nav.Management"
      
        [System.Reflection.Assembly]::Load((New-Object System.Reflection.AssemblyName('Microsoft.SqlServer.SMO, Version=12.0.0.0, Culture=neutral, PublicKeyToken=89845dcd8080cc91'))) | out-null
        [System.Reflection.Assembly]::Load((New-Object System.Reflection.AssemblyName('Microsoft.SqlServer.ConnectionInfo, Version=12.0.0.0, Culture=neutral, PublicKeyToken=89845dcd8080cc91'))) | out-null
    }
    PROCESS 
    {
        Export-NavApplication @PSBoundParameters -ErrorVariable ExportError

        if ($ExportError)
        {
            Write-Error $ExportError[0]
            return
        }

        if ($DatabaseInstance)
        {
            $serverInstanceName = "${DatabaseServer}\${DatabaseInstance}"
        }
        else
        {
            $serverInstanceName = $DatabaseServer
        }

        $sqlServer = ConnectToServer $serverInstanceName
                
        if ($sqlServer.Databases[$DatabaseName] -eq $null)
        {
            Write-Error -Category InvalidArgument "Cannot find database $DatabaseName"
            return
        }

        Write-Verbose "Copying users to destination database $DestinationDatabaseName"
        CopyUsers $sqlServer.Databases[$DatabaseName] $sqlServer.Databases[$DestinationDatabaseName]
    }
}

function ConnectToServer([string]$serverInstanceName) 
{
    $serverConnection = New-Object Microsoft.SqlServer.Management.Common.ServerConnection $serverInstanceName
    $serverConnection.Statementtimeout = 10
    $serverConnection.LoginSecure = $true
    
    return New-Object Microsoft.SqlServer.Management.Smo.Server($serverConnection)
}

function CopyUsers(
    [Microsoft.SqlServer.Management.Smo.Database] $sourceDatabase, 
    [Microsoft.SqlServer.Management.Smo.Database] $targetDatabase)
{
    foreach ($user in $sourceDatabase.Users)
    {
        if ($targetDatabase.Users.Contains($user.Name))
        {
            continue;
        }

        $roles = GetFixedRolesForUser $user

        if ($roles.Count -ne 0)
        {
            $newUser = New-Object  Microsoft.SqlServer.Management.Smo.User($targetDatabase, $user.Name)
...
function GetFixedRolesForUser([Microsoft.SqlServer.Management.Smo.User] $user)
{
    $fixedDatabaseRoles = "db_owner",
                          "db_securityadmin",
                          "db_accessadmin",
                          "db_backupoperator",
                          "db_ddladmin",
    ...
    $tmp = New-Object string[] $user.EnumRoles().Count
    $user.EnumRoles().CopyTo($tmp, 0)

    $roles = New-Object System.Collections.Generic.HashSet[string](,$tmp)    
    $roles.IntersectWith([string[]]$fixedDatabaseRoles)
    return $roles
}

function SafeAddPSSnapin($snapIn)
{
...
Export-ModuleMember -Function HowTo-ExportNAVApplicationDatabase

# SIG # Begin signature block
# MIIa4AYJKoZIhvcNAQcCoIIa0TCCGs0CAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsjNn62WBvx9QqfZkyRVe7Yfr
# e/GgghV6MIIEuzCCA6OgAwIBAgITMwAAAFrtL/TkIJk/OgAAAAAAWjANBgkqhkiG
...
# gjcCAQsxDjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBThVrsMQDybPeyo
# r9A53qX6rfbQHjCBiAYKKwYBBAGCNwIBDDF6MHigWoBYAE0AaQBjAHIAbwBzAG8A
# ZgB0ACAARAB5AG4AYQBtAGkAYwBzACAATgBBAFYAIABDAG8AZABlAHMAaQBnAG4A
# IABTAHUAYgBtAGkAcwBzAHMAaQBvAG4ALqEagBhodHRwOi8vd3d3Lm1pY3Jvc29m
# dC5jb20wDQYJKoZIhvcNAQEBBQAEggEAk3HJFmsdgF1UrrActARKdNzbhdJ2ZNG/
# YbHE9WUQSlAivtIekogs4uj5NHT9+XCU3v1MBk4g2/MRORKm2UUas4wSMrwd2OWW
# yJHFC84uYPqm1imGTQYtLUgqbHfBboJCketyMLb7JQRGj2K3hLRmgz45tu1BnUDF
# PboGbjqvfviH7apMoceOg7c1PqWFbPjVBdG2WVM5Nr94BsUn1Pz99FFNK3I5tFgb
# fT2MYN1g7wB/b2Yh5tYF58WFCEVcmVla+cHDxxbz1t/Hg0rv4+3s9IPkkmDMU4HJ
# 4AWa27p75hRz8UjfQhamPb6nM3X/mgG9amXYwE8lOOL8kS4H/LWNhqGCAigwggIk
# BgkqhkiG9w0BCQYxggIVMIICEQIBATCBjjB3MQswCQYDVQQGEwJVUzETMBEGA1UE
# CBMKV2FzaGluZ3RvbjEQMA4GA1UEBxMHUmVkbW9uZDEeMBwGA1UEChMVTWljcm9z
# b2Z0IENvcnBvcmF0aW9uMSEwHwYDVQQDExhNaWNyb3NvZnQgVGltZS1TdGFtcCBQ
# Q0ECEzMAAABa7S/05CCZPzoAAAAAAFowCQYFKw4DAhoFAKBdMBgGCSqGSIb3DQEJ
# AzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTE1MDEyOTIxNDkxMlowIwYJ
# KoZIhvcNAQkEMRYEFH0ud8tgb5LWIlNlwvpWfvar/o5EMA0GCSqGSIb3DQEBBQUA
# BIIBAK20H/+uZtPGuhP6lXMStKxZKSNtNkpSyL1hgy3eZxd6jXU9TIhYAI5ajAHl
# sT9g91+cGfk+6IJBPBQIZ/gqRcvYBiugoJotIz5PGfvH2DbcNJ9CujSUDfAoIBo1
# andL6mk9emApO4jct7QbpB8JvkCIpeKMbYbTz3EV8Or7mEPCDsVbUjG3AHOfowQi
# evQ0dpY3OvbsSRpM6XpDZGiKkTp3AQLVwhxQ0xEWxgG5TG5w/D0f5alpk012Mx5w
# WoIWHVmaY9w92UbDGmHIr/jg2Rs/IiUDQiP3WMjicS1dx2RUQT6KZEUx2TxPG9HG
# pKNZdRF6SYGZ/CG3gF7UcYMJenQ=
